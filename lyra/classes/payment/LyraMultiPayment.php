...
class LyraMultiPayment extends AbstractLyraPayment
{
    protected $prefix = 'LYRA_MULTI_';
    ...
    public static function getAvailableOptions($cart = null)
    {
        ...
        $options = @unserialize(Configuration::get('LYRA_MULTI_OPTIONS'));
    ...
    public function getTplVars($cart)
    {
        ...
        $vars['lyra_multi_options'] = self::getAvailableOptions($cart);

        $entry_mode = Configuration::get($this->prefix . 'CARD_MODE');
        $vars['lyra_multi_card_mode'] = $entry_mode;
    ...
    {
        $all_cards = LyraTools::getSupportedMultiCardTypes();
