 ...
 */
class LyraSubmitModuleFrontController extends ModuleFrontController
{
    public $ssl = true;
    ...
    public function postProcess()
    {
        ...
        if ($this->iframe) {
            unset($this->context->cookie->lyraCartId); // Used in iframe mode.
        }
    ...
    private function processPaymentReturn()
    {
        ...
        if (! $response->isAuthentified()) {
            ...
            $this->logger->logError("{$ip} tries to access module/lyra/submit page without valid signature with parameters: " . print_r($_REQUEST, true));
        ...
        if ($order_id == false) {
            ...
            if ($response->isAcceptedPayment()) {
                ...
                if ($save_on_failure || Lyra::isOney($response)) {
                    ...
                    $this->lyraRedirect('index.php?controller=history');
                } else {
            ...
            $new_state = (int) Lyra::nextOrderState($response, false, $outofstock);
            ...
            if (! Configuration::get('PS_OS_OUTOFSTOCK_PAID')) {
                $bc_os_state_valid &= Lyra::isStateInArray($new_state, array('PS_OS_OUTOFSTOCK', 'LYRA_OS_PAYMENT_OUTOFSTOCK'));
            }
            ...
            } elseif (Lyra::isStateInArray($old_state, Lyra::getManagedStates())) {
                if (($old_state == Configuration::get('PS_OS_ERROR')) && $response->isAcceptedPayment() && Lyra::hasAmountError($order)) {
