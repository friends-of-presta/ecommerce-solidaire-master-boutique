...
class Lyra extends PaymentModule
{
    ...
    private function refund($order, $amount)
    {
        ...
        try {
            ...
            LyraTools::checkRestResult($getPaymentDetails);
            ...
            $moduleId = array_search($method, LyraTools::$submodules);
            ...
            if ($transStatus === 'CAPTURED') { // Transaction captured, we can do refund.
                ...
                if ($amountInCents >= $transAmount) { // Transaction cancel.
                    ...
                    $captureDelay = Configuration::get('LYRA_' . $moduleId . '_DELAY'); // Get submodule specific param.
                    if (! is_numeric($captureDelay)) {
                        // Get general param.
                        $captureDelay = Configuration::get('LYRA_CAPTURE_DELAY');
                    }

                    $expectedCaptureDate = is_numeric($captureDelay) ? new DateTime('@' . strtotime("+$captureDelay days", $timestamp)) : null;

                    $validationMode = Configuration::get('LYRA_' . $moduleId . '_VALIDATION'); // Get submodule specific param.
                    if ($validationMode === '-1') {
                        // Get general param.
                        $validationMode = Configuration::get('LYRA_VALIDATION_MODE');
                    }
